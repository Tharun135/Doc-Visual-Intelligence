...
def _build_insertion_snippet(visual_type: str, generated_artifact: dict | None, placement_hint: dict | None) -> str | None:
    ...
    if visual_type in {"Screenshot", "Configuration Screenshot", "Annotated Screenshot"}:
        return (
            f"<!-- Insert {visual_type}: {placement} -->\n"
            "![describe-screenshot](../media/your-screenshot.png)"
        )
...
def detect_visuals(title: str, content: str, section_context: dict | None = None) -> list[dict]:
    ...
    for rule in PYTHON_RULES:
        ...
        generated_artifact = _build_generated_artifact(
            result.visual_type, title, content, signals, content_type
        )
        ...
        python_hits.append({
            ...
            "suggested_content": _suggest_content(result.visual_type, title, signals),
        })
    ...
    for hit in json_hits:
        ...
        generated_artifact = _build_generated_artifact(
            hit["visual_type"], title, content, signals, content_type
        )
        ...
        hit.update({
            ...
            "generated_artifact": generated_artifact,
            "insertion_snippet": insertion_snippet,
            "suggested_content": _suggest_content(hit["visual_type"], title, signals),
        })
...
def _suggest_content(visual_type: str, title: str, signals: dict) -> str:
    ...
    if visual_type in {"Screenshot", "Configuration Screenshot", "Annotated Screenshot"}:
        return "Capture the UI state after the main interaction. Annotate the active field or button."
